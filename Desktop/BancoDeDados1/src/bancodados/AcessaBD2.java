...
public class AcessaBD2 extends java.awt.Frame {
    ...
    public boolean AcessaBD() {
        try {
            ...
            String urlconexao = "jdbc:postgresql://127.0.0.1/banco_java_desktop"; //para acesso ao banco de dados fabricio, usando o banco de dados Postgre.
    ...
    //P Update porque Prepare Statement
    public void ExecutaPUpdate() {
        try {
            String sqldml = 
                "INSERT INTO clientes (nome, fone, email, idade) " +
                "values (?, ?, ?, ?)";
    ...
    public void ExecutaStoredProcedure1() {
        try {
            int tipo = ResultSet.TYPE_SCROLL_SENSITIVE;//(c)
            int concorrencia = ResultSet.CONCUR_UPDATABLE;
            cstdados = connection.prepareCall("{call consultaclientesemail()}", tipo, concorrencia);
    ...
    public void ExecutaStoredProcedure2() {
        try {
            int tipo = ResultSet.TYPE_SCROLL_SENSITIVE;//(c)
            int concorrencia = ResultSet.CONCUR_UPDATABLE;
            cstdados = connection.prepareCall("{call consultamaioridade(?)}", tipo, concorrencia);
